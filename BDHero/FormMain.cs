...
using BDHero.Properties;
using BDHero.Services;
using BDHero.controllers;
using BDHero.models;
using BDHero.Views;
using BDHeroCore;
using BDHeroCore.BDInfo;
using BDHeroCore.Services;
using BDHeroCore.Tools;
using DotNetUtils;
...
namespace BDHero
{
    public partial class FormMain : Form
    {
        ...
        public FormMain(string[] args)
        {
            ...
            {
                textBoxSource.Text = BDHeroSettings.LastPath;
            }
        ...
        {
            if (BDHeroSettings.DetailsWindowMaximized)
                WindowState = FormWindowState.Maximized;

            if (BDHeroSettings.DetailsWindowLocation != Point.Empty)
                Location = BDHeroSettings.DetailsWindowLocation;

            if (BDHeroSettings.DetailsWindowSize != Size.Empty)
                Size = BDHeroSettings.DetailsWindowSize;

            Text = BDHeroSettings.AssemblyName + " v" + BDHeroSettings.AssemblyVersionDisplay;
            ...
            textBoxOutputDir.Text = BDHeroSettings.OutputDir;
            textBoxOutputFileName.Text = BDHeroSettings.OutputFileName;

            textBoxReplaceSpaces.Text = BDHeroSettings.ReplaceSpacesWith;
            checkBoxReplaceSpaces.Checked = BDHeroSettings.ReplaceSpaces;

            checkBoxDemuxLPCM.Checked = BDHeroSettings.DemuxLPCM;
            checkBoxDemuxSubtitles.Checked = BDHeroSettings.DemuxSubtitles;
        ...
        private void Init(BDROM bdrom, ICollection<Language> languages, List<TSPlaylistFile> playlists)
        {
            ...
            if (BDHeroSettings.UseMainMovieDb)
                QueryMainMovie();
            else
                SearchTmdb();
        ...
        {
            _isScanningBDROM = false;
            BDHeroSettings.LastPath = textBoxSource.Text;
            BDHeroSettings.SaveSettings();
            ResetUI();
        }
        ...
        private void PopulateOutputTab()
        {
            ...
            Language userPrefAudienceLanguage = BDHeroSettings.AudienceLanguage;
        ...
        private void PopulateAudioTracks()
        {
            var i = 0;
            var icons = new ImageList { ColorDepth = ColorDepth.Depth32Bit };
            var preferredAudioCodecs = BDHeroSettings.PreferredAudioCodecs;
            var minChannelCount = 0;

            if (BDHeroSettings.SelectHighestChannelCount)
            {
        ...
        private void Rip()
        {
            ...
            */
            ...
            BDHeroSettings.OutputDir = textBoxOutputDir.Text;
            BDHeroSettings.OutputFileName = textBoxOutputFileName.Text;
            BDHeroSettings.ReplaceSpaces = checkBoxReplaceSpaces.Checked;
            BDHeroSettings.ReplaceSpacesWith = textBoxReplaceSpaces.Text;
            BDHeroSettings.DemuxLPCM = checkBoxDemuxLPCM.Checked;
            BDHeroSettings.DemuxSubtitles = checkBoxDemuxSubtitles.Checked;
            BDHeroSettings.SaveSettings();
        ...
        private bool CanSubmitToDb
        {
            get
            {
                if (!_initialized)
                    return false;
                if (!String.IsNullOrEmpty(BDHeroSettings.ApiKey) && searchResultListView.SelectedIndices.Count > 0)
                {
                    if (_autoConfigured)
                    {
        ...
        private void SetAudienceLanguage(IEnumerable<Language> array, ListControl control)
        {
            ...
            {
                MessageBox.Show(this, ex.Message, "BDHero Error",
            ...
            EventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show(msg, "BDHero Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void FormDetails_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (!e.Cancel)
            {
                ...
                {
                    BDHeroSettings.DetailsWindowLocation = RestoreBounds.Location;
                    BDHeroSettings.DetailsWindowSize = RestoreBounds.Size;
                    BDHeroSettings.DetailsWindowMaximized = true;
                }
                else
                {
                    BDHeroSettings.DetailsWindowLocation = Location;
                    BDHeroSettings.DetailsWindowSize = Size;
                    BDHeroSettings.DetailsWindowMaximized = false;
                }
        ...
        {
            if (BDHeroSettings.CheckForUpdates && !ClickOnceUpdateService.IsClickOnce)
            {
                checkForUpdatesToolStripMenuItem.Enabled = false;
